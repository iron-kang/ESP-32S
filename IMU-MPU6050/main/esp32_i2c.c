...
esp_err_t _writeBits(I2C_CONFIG *self, int8_t regAddr, uint8_t bitStart, uint8_t length, uint8_t data);
esp_err_t _writeBit(I2C_CONFIG *self, uint8_t regAddr, uint8_t bitNum, uint8_t data);
esp_err_t _read(I2C_CONFIG *self, uint8_t addr, uint8_t *data_rd, size_t size);
esp_err_t _readBits(I2C_CONFIG *self, uint8_t regAddr, uint8_t bitStart, uint8_t length, uint8_t *data);
...
{
    uint8_t buf[2];
    if (self->read(self, regAddr, &buf[1], 1) == ESP_OK)
    {
...
{
    uint8_t b;
    if (self->read(self, regAddr, &b, 1) == ESP_OK) {
        uint8_t mask = ((1 << length) - 1) << (bitStart - length + 1);
        b &= mask;
        b >>= (bitStart - length + 1);
        *data = b;
