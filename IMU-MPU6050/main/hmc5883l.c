...
{
    hmc->i2c.ADDR  = HMC5883L_ADDRESS;
    hmc->i2c.MODE  = I2C_MODE_MASTER;
    hmc->i2c.SDA   = hmc->SDAPin;
    hmc->i2c.SCL   = hmc->SCLPin;
    hmc->i2c.SPEED = 400000;
    i2c_init(&hmc->i2c);
    ...
    hmc->setDataRate(hmc, HMC5883L_DATARATE_15HZ);
    hmc->setSamples(hmc, HMC5883L_SAMPLES_1);
...
Vector _readRaw(HMC5883L *self)
{
    self->v.XAxis = readRegister16(&self->i2c, HMC5883L_REG_OUT_X_M) - self->xOffset;
    self->v.YAxis = readRegister16(&self->i2c, HMC5883L_REG_OUT_Y_M) - self->yOffset;
    self->v.ZAxis = readRegister16(&self->i2c, HMC5883L_REG_OUT_Z_M);
...
Vector _readNormalize(HMC5883L *self)
{
    self->v.XAxis = ((float)readRegister16(&self->i2c, HMC5883L_REG_OUT_X_M) - self->xOffset) * self->mgPerDigit;
    self->v.YAxis = ((float)readRegister16(&self->i2c, HMC5883L_REG_OUT_Y_M) - self->yOffset) * self->mgPerDigit;
    self->v.ZAxis = (float)readRegister16(&self->i2c, HMC5883L_REG_OUT_Z_M) * self->mgPerDigit;
...
{
    uint8_t value;
    if (i2c->read(i2c, reg, &value, 1) != ESP_OK)
        return ESP_FAIL;
    return value;
}
...
int16_t readRegister16(I2C_CONFIG *i2c, uint8_t reg)
{
    int16_t value;
    uint8_t buf[2];
    if (i2c->read(i2c, reg, buf, 2) != ESP_OK)
        return ESP_FAIL;
